		...
		try {
			...
			if (this.scriptFile == null) {
				try {
					...
					this.scriptFile.write("0" + sep + "version" + sep
							+ RunGame.versionNum + "\n");
				} catch (IOException e) {
			...
			while (i <= halfTrials / 2) {
				g.trials.add(i, new Trial(g.good.soundFile, g.good.throbRate,
						true, (rand.nextInt(2) == 1), Species.good));
				i++;
			}
			// congruent bad fishes
			while (i <= halfTrials) {
				g.trials.add(i, new Trial(g.bad.soundFile, g.bad.throbRate,
						true, (rand.nextInt(2) == 1), Species.bad));
				i++;
			}
			// incongruent bad fishes
			while (i <= halfTrials / 2 + halfTrials) {
				g.trials.add(i, new Trial(g.good.soundFile, g.bad.throbRate,
						false, (rand.nextInt(2) == 1), Species.bad));

				i++;
			}
			// incongruent good fishes
			while (i <= halfTrials) {
				g.trials.add(i, new Trial(g.bad.soundFile, g.good.throbRate,
						true, (rand.nextInt(2) == 1), Species.good));
				i++;
			}
			// shuffle

...
/**
 * This class is a port to Java of the Matlab NetStation function. It allows the
 * client to create a connection to a NetStation machine and to send various
...
public class NetStation {
	...
	/**
	 * Open a connection to the NetStation. on the host/port stored when the
	 * NetStation object was created This creates the socket, gets the input and
	 * output streams Sends the initialization string "QMAC-" to the NetStation
	 * and receives the "all good" two byte response ('I' 1) If anything goes
	 * wrong it throws an error and there are lots of places for something to go
	 * wrong!
	 * 
	...
	public void connectNS() throws NetStationError, InterruptedException,
		...
		try {
			...
			throw new NetStationError(con,
					"problem sending QMAC- identifier to NetStation, e=" + e);
		}
		...
		if (b != 'I') {
			throw new NetStationError(con,
					"NetStation ECI error: didn't like QMAC- message, expected "
							+ 'I' + " as acknowledgement but found " + b);
		}
		if (vers != 1) {
			throw new NetStationError(con,
	...
	/**
	 * Disconnect the proxy from the NetStation. It does this by sending the
	 * message "EX" and reading the two byte response. It also waits a half
	 * second or so before sending each character.
	 * 
	 * @throws InterruptedException
	 * @throws IOException
	 */
	...
	/**
	 * Synchronize clocks with the NetStation. This attempts to synchronize the
	 * time with the NetStation. So that they both agree on the current time
	 * within a few milliseconds It does this by
	 * <ul>
	 * <li>
	 * sending 'X' and receiving a character</li>
	 * <li>
	 * calculating some representation of the current time x in milliseconds,
	 * for example, where x is the number of ms since the connection was made</li>
	 * <li>
	 * sending the five bytes ('T' x0 x1 x2 x3) where x0 x1 x2 x3 is the four
	 * byte little endian representation of x</li>
	 * <li>
	 * reading a response from the NetStation and checking that no more that 2.5
	 * ms have elapsed since the time x</li>
	 * <li>
	 * if more than 2.5 seconds have elapsed, then it repeats the process up to
	 * 100 times before giving up</li>
	 * </ul>
	 * 
	 * 
	 * @throws IOException
	...
	/**
	 * Tell the NetStation to start recording by sending 'B' and receiving a
	 * byte
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Tell the NetStation to stop recording by sending 'E' and receiving a byte
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Sends an event to the NetStation You have to be careful to send data in
	 * little endian form..
	 * 
	 * @param code
	 *            a 4 character event code
	 * @param startMS
	 *            the time that the event starts
	 * @param durationMS
	 *            the duration of the event
	 * @param keyCodes
	 *            an array of 4 character key codes
	 * @param keyValues
	 *            an array of unsigned 16 bit integer values
	 * @throws IOException
	 */
	public void eventNS(String code, int startMS, int durationMS,
		...
		out.write('D');
		out.write(int_to_big_endian2(numBytes)); // num bytes to send after this
													// line
		...
		for (int i = 0; i < keyvaluepairs; i++) {
			...
			out.write(int_to_big_endian2(2)); // 2 byte representation of number
												// of bytes for keyValue
	...
	/**
	 * Run a demo that connects with the NetStation, sends some events, and
	 * disconnects.
	 * 
	 * @param args
	 */

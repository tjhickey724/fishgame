...
public class GameView extends JPanel {
	...
	private void drawBackground(Graphics g) {
		...
		double frames = seconds * 0.1; // 0.1 should be background speed, a
										// variable ...
		...
		// draw image on screen tiled REFACTOR
	...
	private void drawActor(Graphics g, GameActor a, Color c) {
		...
		int theHeight = theSize * fishL.getHeight() / fishL.getWidth(); // (int)
																		// ((theSize
																		// *
																		// aspectRatio)/100);
		if (a.fromLeft) {
			g.drawImage(fishL, x - theWidth / 2, y - theHeight / 2, theWidth,
					theHeight, null);
		} else {
			g.drawImage(fishR, x - theWidth / 2, y - theHeight / 2, theWidth,
					theHeight, null);
		}
